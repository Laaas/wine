...
static void *allocate_large_block( HEAP *heap, DWORD flags, SIZE_T size )
{
    ...
    if (virtual_alloc( NtCurrentProcess(), &address, 0,
                       &block_size, MEM_COMMIT, get_protection_type( flags ), 5 ))
    {
...
void heap_set_debug_flags( HANDLE handle )
{
        ...
        ((flags & HEAP_FREE_CHECKING_ENABLED) || RUNNING_ON_VALGRIND))
    {
        ...
        if (!virtual_alloc( NtCurrentProcess(), &ptr, 0, &size, MEM_COMMIT, PAGE_READWRITE, 4 ))
        {
